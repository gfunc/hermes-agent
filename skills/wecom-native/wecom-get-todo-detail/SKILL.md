...
使用 `mcp_wecom_contact_get_userlist` 原生 MCP 工具 `{}` 获取通讯录，在返回的 userlist 中，用 userid 字段匹配 follower_id / creator_id，取对应的 name。
...
1. **人员 ID 必须转姓名**
   ...
   - 展示待办详情前，先使用 `mcp_wecom_contact_get_userlist` 原生 MCP 工具 `{}` 获取通讯录
