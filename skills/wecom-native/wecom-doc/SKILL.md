...
当 MCP 工具返回结果中 `errcode` 不为 0 且 `help_message` 有值时，将 `help_message` 中面向用户的内容输出给用户。
...
2. 输出后**等待用户回复**，后续流程与「MCP Server 未配置」的场景 A / B / C 一致。配置成功后**不要停下来**，立即重新验证 `mcp_wecom_doc_list` 是否可用，验证通过后继续执行用户的原始请求。

### MCP Server 未配置

如果 `mcp_wecom_doc_*` 或 `mcp_wecom_doc_list` 返回 **server not found**、**unknown server** 或类似错误，说明 wecom-doc MCP server 尚未配置。

#### 排查步骤

1. 先检查 WeCom 通道是否已连接（参考 `wecom-preflight` 技能）
2. 若已连接但工具仍不可用，可尝试发送 `/reload-mcp` 刷新 MCP 配置
3. 若刷新后仍不可用，可按以下方式手动配置：
