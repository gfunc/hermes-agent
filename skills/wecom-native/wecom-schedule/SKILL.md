...
调用 `mcp_wecom_schedule_get_schedule_list_by_range '{"start_time": "YYYY-MM-DD HH:MM:SS", "end_time": "YYYY-MM-DD HH:MM:SS"}'`

返回 `schedule_id_list` 数组。仅支持当日前后 30 天。

### get_schedule_detail — 获取日程详情

调用 `mcp_wecom_schedule_get_schedule_detail '{"schedule_id_list": ["SCHEDULE_ID_1", "SCHEDULE_ID_2"]}'`

支持 1~50 个 ID，返回日程标题、时间、地点、参与者等。参见 [API 详情](references/api-get-schedule-detail.md)。

### create_schedule — 创建日程

调用 `mcp_wecom_schedule_create_schedule '{"schedule": {"start_time": "YYYY-MM-DD HH:MM:SS", "end_time": "YYYY-MM-DD HH:MM:SS", "summary": "日程标题", "attendees": [{"userid": "USER_ID"}], "reminders": {"is_remind": 1, "remind_before_event_secs": 3600, "timezone": 8}}}'`
...
调用 `mcp_wecom_schedule_update_schedule '{"schedule": {"schedule_id": "SCHEDULE_ID", "summary": "更新后的标题"}}'`

参见 [API 详情](references/api-update-schedule.md)。

### cancel_schedule — 取消日程

调用 `mcp_wecom_schedule_cancel_schedule '{"schedule_id": "SCHEDULE_ID"}'`

### add_schedule_attendees / del_schedule_attendees — 管理参与人

- 添加参与人：调用 `mcp_wecom_schedule_add_schedule_attendees '{"schedule_id": "SCHEDULE_ID", "attendees": [{"userid": "USER_ID"}]}'`
- 移除参与人：调用 `mcp_wecom_schedule_del_schedule_attendees '{"schedule_id": "SCHEDULE_ID", "attendees": [{"userid": "USER_ID"}]}'`

### check_availability — 查询闲忙

调用 `mcp_wecom_schedule_check_availability '{"check_user_list": ["USER_ID_1", "USER_ID_2"], "start_time": "YYYY-MM-DD HH:MM:SS", "end_time": "YYYY-MM-DD HH:MM:SS"}'`
